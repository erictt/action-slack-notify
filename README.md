...
| Output      | Description                                                                                                |
| ----------- | ---------------------------------------------------------------------------------------------------------- |
| `thread_ts` | The timestamp (ts) of the message sent to Slack. This can be used as `SLACK_THREAD_TS` for sending replies in a thread. |

### Example: Using `thread_ts` to reply in a thread

You can use the `thread_ts` output from one step as an input to a subsequent step to create a threaded conversation.

```yml
on: push
name: Slack Threaded Notification Demo
jobs:
  slackNotification:
    name: Slack Notification
    runs-on: ubuntu-latest
    ...
    - name: Send Initial Slack Message
      id: initial_message # Give an ID to this step to access its outputs
      uses: rtCamp/action-slack-notify@v2
      env:
        SLACK_WEBHOOK: ${{ secrets.SLACK_WEBHOOK }}
        SLACK_MESSAGE: "This is the first message in the thread."
        SLACK_CHANNEL: "your-channel" # Optional if configured in webhook

    - name: Send Reply in Thread
      if: success() # Only run if the initial message was sent successfully
      uses: rtCamp/action-slack-notify@v2
      env:
        SLACK_WEBHOOK: ${{ secrets.SLACK_WEBHOOK }}
        SLACK_MESSAGE: "This is a reply to the first message."
        SLACK_CHANNEL: "your-channel" # Optional if configured in webhook
        SLACK_THREAD_TS: ${{ steps.initial_message.outputs.thread_ts }} # Use the output here
```
